# Customer Proxy Validation Track

Use this runbook to validate `gametau` for closed-source customer ports without needing access to private code.

## Purpose

- Validate customer integration workflows end-to-end using only public repo assets.
- Stress the same seams a private game port will use (`commands.rs`, typed frontend wrappers, runtime bridge, persistence/eventing).
- Produce an evidence bundle that release gates can consume.

## Scope Boundaries

- Requirements can be derived from open references (for example Solar Tycoon `origin/feature/godot`).
- Validation target is `gametau` products: runtime bridge, templates, docs, and release process.
- Execution of the real game port is outside this repository in a private customer codebase.

## Validation Assets

- Open showcase baseline: `examples/battlestation`
- Fresh scaffold baseline: `bunx create-gametau customer-proxy`
...
```bash
bunx create-gametau customer-proxy
cd customer-proxy
bun install
bun run dev
bun run dev:tauri
...
- Cancellation stops further state mutation.
...
- `task.test.ts` output confirming all 17 lifecycle tests pass.
